...
import {
  ...
  ShoppingBag, Zap, BookOpen, Shield, ChevronRight,
} from "lucide-react";
...
const RANK_COLORS: Record<string, string> = {
  ...
  National: "text-rank-national", Monarch: "text-monarch-purple",
};

export function Sidebar() {
  const pathname = usePathname();
  const { hunterName, rank, level, currentExp, maxExp, streakDays } = useAppStore();
    ...
    <>
      ...
      <aside className="hidden md:flex fixed left-0 top-0 h-full w-64 flex-col z-40
        ...
        <div className="p-5 border-b border-system-blue/20">
          ...
          <p className="text-system-blue/60 font-mono text-xs">GYM PROTOCOL v1.0</p>
        </div>
          ...
          <div className="system-panel rounded-lg p-3">
            <div className="system-panel-inner">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-monarch-purple to-shadow-mid
                  border border-system-blue/30 flex items-center justify-center text-lg">
                  🌑
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-cinzel text-white text-sm font-bold truncate">{hunterName}</p>
                  <div className="flex items-center gap-2">
                    <span className={clsx("font-mono text-xs font-bold", RANK_COLORS[rank])}>
                      [{rank}-RANK]
                    </span>
                    <span className="font-mono text-system-blue/60 text-xs">Lv.{level}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-2 flex items-center gap-1">
                ...
                <span className="font-mono text-xs text-warning">{streakDays} day streak</span>
              </div>
            </div>
          </div>
        </div>
            ...
            return (
              <Link key={href} href={href}>
                ...
                >
                  ...
                  <Icon size={16} className={clsx("flex-shrink-0 relative z-10", active && "animate-glow-pulse")} />
                  <div className="flex-1 relative z-10">
                    <p className="text-sm font-medium leading-none">{label}</p>
                    <p className="text-xs text-slate-500 group-hover:text-slate-400 mt-0.5">{sub}</p>
                  </div>
                  {active && <ChevronRight size={12} className="text-system-blue relative z-10" />}
                </motion.div>
              </Link>
            );
          })}
