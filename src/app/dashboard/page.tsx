    ...
    <div className="relative min-h-screen bg-void p-6 md:p-8">
      ...
      <div className="relative z-10 max-w-6xl mx-auto space-y-6">
        ...
        >
          <div>
            ...
            <h1 className="font-cinzel text-3xl font-black text-white tracking-wide">
              Welcome back,{" "}
              <span className="text-system-blue">{hunterName}</span>
            </h1>
            <p className="font-mono text-sm text-slate-400 mt-1 italic">
              &quot;{title}&quot;
            </p>
          </div>
              ...
              <div className="p-4 space-y-4">
                ...
                <div className="flex items-center gap-4">
                  <div className="relative">
                    <div
                      className="w-16 h-16 rounded-full bg-gradient-to-br from-monarch-purple via-shadow-mid to-void
                      border-2 border-system-blue/40 flex items-center justify-center text-3xl shadow-system animate-float"
                    >
                      🌑
                    </div>
                    <div
                      className="absolute -bottom-1 -right-1 w-6 h-6 bg-shadow-dark rounded-full border border-system-blue/40
                ...
                <div>
                  ...
                  <p className="font-mono text-xs text-slate-500 mt-1 text-right">
                    {expPct}% to Level {level + 1}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Link href="/dungeon">
                    <button
                      className="w-full py-2 px-3 rounded-lg bg-system-blue/10 border border-system-blue/30
                  ...
                  <Link href="/status">
                    <button
                      className="w-full py-2 px-3 rounded-lg bg-monarch-purple/10 border border-monarch-purple/30
              ...
              <div className="p-4 space-y-3">
                ...
                <div className="pt-2 border-t border-system-blue/20 flex justify-between items-center">
                  ...
                  <span className="font-mono text-sm font-bold text-gold">
                    {useAppStore.getState().statPoints} pts
                  </span>
                </div>
              </div>
            </SystemPanel>
              ...
              <div className="p-4">
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-4">
                  <div>
                    ...
                    <p className="font-mono text-xs text-slate-400 mt-1 italic">
                      {mainQuest.systemMessage}
                    </p>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <Clock size={14} className="text-warning" />
                    <span className="font-mono text-xs text-warning">
                      {mainQuest.expiresIn}h remaining
                    </span>
                  </div>
                </div>
                    ...
                    return (
                      <div key={i}>
                        <div className="flex justify-between font-mono text-xs text-slate-300 mb-1">
                          <span>◈ {req.label}</span>
                          <span>
                            {req.current} / {req.total} {req.unit}
                          </span>
                        </div>
                ...
                <div className="mt-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <span className="font-mono text-xs text-success">
                      +{mainQuest.reward.exp} EXP
                    </span>
                    <span className="font-mono text-xs text-gold">
                      +{mainQuest.reward.gold} G
                    </span>
                    {mainQuest.reward.stat && (
                      <span className="font-mono text-xs text-monarch-light">
                        {mainQuest.reward.stat}
                      </span>
                    )}
                  </div>
                  <Link href="/dungeon">
                    <button
                      className="px-4 py-2 rounded-lg bg-monarch-purple/20 border border-monarch-purple/40
              ...
              <div className="p-4 space-y-3">
                {sideQuests.map((q) => {
                  ...
                  return (
                    <div
                      key={q.id}
                      className="p-3 rounded-lg bg-shadow-mid/50 border border-system-blue/10 hover:border-system-blue/30 transition-all"
                    >
                ...
                <div className="grid grid-cols-3 gap-2">
                  {shadowArmy.slice(0, 6).map((s) => (
                    ...
                    >
                      ...
                      <p className="font-mono text-xs text-system-blue">
                        Lv.{s.level}
                      </p>
        ...
        >
          <button
            onClick={() =>
              toast.success("Shadow soldier Igris has leveled up! Lv.7 → Lv.8")
            }
          ...
          <button
            onClick={() =>
              toast.warn(
                "Daily quest expires in 2 hours. Complete or face the Penalty Zone.",
              )
            }
          ...
          <button
            onClick={() =>
              toast.danger(
                "EMERGENCY QUEST ISSUED. The System demands action NOW.",
              )
            }
