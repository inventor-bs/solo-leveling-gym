...
export function SystemPanel({
  children, className, glowColor = "blue", corners = true, header,
}: SystemPanelProps) {
  return (
    <div className={clsx("system-panel rounded-lg border", GLOW[glowColor], className)}>
      {corners && (
        <>
          {/* Corner decorations */}
      ...
      <div className="system-panel-inner">
        {header && (
          <div className="px-4 py-2 border-b border-system-blue/20">
            <h3 className="font-mono text-xs text-system-blue tracking-widest uppercase">{header}</h3>
          </div>
        )}
        {children}
      </div>
